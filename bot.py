...
<head>
    ...
    <title>RAJA AI PREMIUM - SMART TRADING DASHBOARD</title>
    ...
    <style id="raja-dashboard-v5">
        ...
        .holo-radar-beam { position:absolute; z-index:3; width:190px; height:190px; border-radius:50%; background:conic-gradient(from 0deg,transparent 0 77%,rgba(0,255,135,.05) 80%,rgba(96,239,255,.7) 87%,transparent 90%); animation:scannerOrbit 2.8s linear infinite; filter:drop-shadow(0 0 8px #60efff); }
    ...
    <style id="raja-v9-premium-ui">
        ...
        .scan-telemetry { grid-column:1/-1; display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:6px; margin-top:2px; }
    ...
    <style id="raja-v13-timer-moved">
        ...
        .mini-chart-wrap { position:relative; width:100%; height:170px; margin:8px 0; border:1px solid rgba(96,239,255,.22); background:linear-gradient(180deg,#030914,#06111e); border-radius:10px; overflow:hidden; }
        .mini-chart-wrap canvas { width:100%; height:100%; display:block; }
        .mini-chart-label { position:absolute; top:6px; left:8px; color:#60efff; font-size:8px; font-weight:950; letter-spacing:.6px; text-transform:uppercase; z-index:2; }
        ...
        .chart-legend { position:absolute; right:8px; top:6px; display:flex; gap:6px; z-index:2; font-size:7px; font-weight:900; }
...
<body>
    ...
    <div class="container">
                ...
                <main class="main-column">
                    <div class="dashboard-topbar">
                        ...
                        <div class="top-actions">
                            ...
                            <button id="installAppBtn" class="top-btn install-btn" onclick="installPWA()">⬇ INSTALL APP</button>
                        ...
                        <div id="hudBoxContainer" class="ai-hud-box">
                            ...
                            <div id="signalView" class="signal-view">
                                <div id="signal3dShell" class="premium-signal-result signal-live-buy">
                                    ...
                                    <div class="mini-chart-wrap"><div class="mini-chart-label">LIVE CANDLE PREVIEW · ENTRY LINE</div><div class="chart-legend"><span style="color:#00ff87">UP</span><span style="color:#ff5d7d">DOWN</span></div><canvas id="signalMiniChart"></canvas></div>
                            ...
                            <div id="noSignalView" class="no-signal-view">
                                ...
                                <div style="font-size:10px;color:#a9bad0;margin-bottom:12px;" id="noSignalDesc">Current filters did not converge safely.</div>
                            ...
                            <div id="replayInlineView" class="inline-content-view">
                                ...
                                <div class="mini-chart-wrap"><div class="mini-chart-label">SIGNAL SNAPSHOT + OUTCOME</div><canvas id="replayMiniChart"></canvas></div>
        ...
        <div id="adminPanel" class="panel-box">
            ...
            <div style="margin-top: 10px; border-top: 1px solid rgba(0, 242, 254, 0.3); padding-top: 8px;">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:6px;"><select id="adminPlan" class="input-field"><option value="VIP">VIP</option><option value="VIP GOLD">VIP GOLD</option><option value="VIP PLATINUM">VIP PLATINUM</option></select><select id="adminDurationDays" class="input-field"><option value="0">Lifetime</option><option value="7">7 Days</option><option value="30">30 Days</option><option value="90">90 Days</option><option value="365">365 Days</option></select></div>
                <button class="action-btn" onclick="generateKey()">GENERATE NEW LICENSE KEY</button>
    ...
    <script>
        ...
        window.addEventListener('beforeinstallprompt', (event) => { event.preventDefault(); deferredInstallPrompt=event; const b=document.getElementById('installAppBtn'); if(b)b.style.display='inline-flex'; });
        async function installPWA(){ if(!deferredInstallPrompt){ alert('On mobile, use your browser menu and choose “Install app” or “Add to Home Screen”.'); return; } deferredInstallPrompt.prompt(); await deferredInstallPrompt.userChoice; deferredInstallPrompt=null; const b=document.getElementById('installAppBtn'); if(b)b.style.display='none'; }
        ...
        function setScanProgress(percent) {
            ...
            if (scanSessionState.running && scanSessionState.startedAt) {
                ...
                if (elapsedEl) elapsedEl.innerText = String(Math.floor(elapsed / 60)).padStart(2,'0') + ':' + String(elapsed % 60).padStart(2,'0');
            }
        ...
        async function checkRoute() {
            ...
            if (isUserVerified) {
                ...
                setScanMode(scanMode); hydrateUpgradeSettings(); await refreshTrackedHistory(); loadUserProfile();
        ...
        async function generateKey() {
            ...
            const plan=document.getElementById('adminPlan')?.value||'VIP', duration_days=Number(document.getElementById('adminDurationDays')?.value||0);
        ...
        async function refreshTrackedHistory() {
            try { const auth=getAuthPayload(); if(!auth.key||!auth.session_token)return; const q=new URLSearchParams({...auth,limit:'60'}); const response=await fetch('/signals/history?'+q.toString()); const result=await response.json(); if(response.ok&&result&&result.status==='success'){backendTrackedHistory=Array.isArray(result.data)?result.data:[];renderHistoryList();updateDailyPerformance();renderPerformanceHeatmap();renderReplaySpotlight();} }
            catch(error){console.error('History refresh error:',error);}
        }
        ...
        function renderSignalChart(canvas, candles, isBuy=true, entryPrice=null, exitPrice=null) {
            if(!canvas||!Array.isArray(candles)||candles.length<2)return; const rect=canvas.getBoundingClientRect(); const dpr=Math.max(1,window.devicePixelRatio||1); canvas.width=Math.max(320,Math.floor(rect.width*dpr));canvas.height=Math.max(120,Math.floor(rect.height*dpr));const ctx=canvas.getContext('2d');ctx.setTransform(dpr,0,0,dpr,0,0);const w=canvas.width/dpr,h=canvas.height/dpr;ctx.clearRect(0,0,w,h);const pad={l:12,r:12,t:24,b:16};const lows=candles.map(c=>Number(c.l)),highs=candles.map(c=>Number(c.h));if(entryPrice)lows.push(Number(entryPrice)),highs.push(Number(entryPrice));if(exitPrice)lows.push(Number(exitPrice)),highs.push(Number(exitPrice));let lo=Math.min(...lows),hi=Math.max(...highs);if(!Number.isFinite(lo)||!Number.isFinite(hi)||hi<=lo)return;const range=(hi-lo)||1;lo-=range*.08;hi+=range*.08;const y=p=>pad.t+(hi-p)/(hi-lo)*(h-pad.t-pad.b);ctx.strokeStyle='rgba(96,239,255,.10)';ctx.lineWidth=1;for(let i=0;i<5;i++){const yy=pad.t+i*(h-pad.t-pad.b)/4;ctx.beginPath();ctx.moveTo(pad.l,yy);ctx.lineTo(w-pad.r,yy);ctx.stroke();}const step=(w-pad.l-pad.r)/candles.length,body=Math.max(2,Math.min(9,step*.55));candles.forEach((c,i)=>{const x=pad.l+step*(i+.5),o=Number(c.o),cl=Number(c.c),hh=Number(c.h),ll=Number(c.l),up=cl>=o;ctx.strokeStyle=up?'#00ff87':'#ff5d7d';ctx.fillStyle=up?'#00ff87':'#ff5d7d';ctx.beginPath();ctx.moveTo(x,y(hh));ctx.lineTo(x,y(ll));ctx.stroke();const top=Math.min(y(o),y(cl)),bh=Math.max(1,Math.abs(y(o)-y(cl)));ctx.fillRect(x-body/2,top,body,bh);});const last=Number(candles[candles.length-1].c);const entry=entryPrice?Number(entryPrice):last;ctx.setLineDash([5,4]);ctx.strokeStyle=isBuy?'#00ff87':'#ff5d7d';ctx.beginPath();ctx.moveTo(pad.l,y(entry));ctx.lineTo(w-pad.r,y(entry));ctx.stroke();ctx.setLineDash([]);ctx.fillStyle=isBuy?'#00ff87':'#ff5d7d';ctx.font='bold 9px Segoe UI';ctx.fillText(isBuy?'▲ ENTRY / UP':'▼ ENTRY / DOWN',pad.l+4,Math.max(12,y(entry)-5));if(exitPrice){ctx.strokeStyle='#facc15';ctx.setLineDash([3,3]);ctx.beginPath();ctx.moveTo(pad.l,y(Number(exitPrice)));ctx.lineTo(w-pad.r,y(Number(exitPrice)));ctx.stroke();ctx.setLineDash([]);ctx.fillStyle='#facc15';ctx.fillText('EXIT',w-pad.r-30,Math.max(12,y(Number(exitPrice))-4));}}
        ...
        document.addEventListener('DOMContentLoaded', () => { installHistoryMirror(); updateAutoRescanUI(); syncSelectedExpiryUI(); calculateRisk(); hydrateUpgradeSettings(); renderPremiumMarketSnapshot(); renderPerformanceHeatmap(); renderReplaySpotlight(); updateSetupBuildMeterProgress(0); });
